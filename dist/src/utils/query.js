...
export async function runQuery(sections, query) {
...
async function detectIntentWithAI(query) {
...
async function generateSummaryWithAI(sections) {
    const prompt = `Provide a concise summary of the following document sections based on the document and if the answer doesn't present in the document say "NOT PRESENT IN DOCUMENT": ${sections.map((section) => `Title: ${section.title}\nContent: ${section.content}`).join('\n\n')}`;
    // const response = await client.chat.completions.create({
    //     model: "deepseek-chat",
    // messages: [
    //     {
...
async function searchInSections(sections, query) {
    const prompt = `Based on the following document sections, answer the user query. If the answer is not present in the document, respond with "NOT PRESENT IN DOCUMENT": ${sections.map((section) => `Content: ${section.content}`).join('\n\n')}\n\nUser Query: ${query}`;
    try {
        const res = await fetch(`https://api.cloudflare.com/client/v4/accounts/${process.env.YOUR_ACCOUNT_ID}/ai/run/@cf/meta/llama-3-8b-instruct`, {
            method: "POST",
            headers: {
